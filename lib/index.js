...
class Client {
  ...
  createSession(onSessionCreated, onClientJoined, onDataSent) {
    ...
    this.wsc.on('message', data => {
      ...
      if (response.command === 'SESSION_CREATED') {
        ...
        if (onClientJoined) onClientJoined(response);
      } else if (response.command === 'DATA_SENT') {
        if (onDataSent) onDataSent(response);
      }
    });
  }
  ...
  joinSession(onSessionJoined, onDataSent) {
    ...
    this.wsc.on('message', data => {
      ...
      if (response.command === 'SESSION_JOINED') {
        if (onSessionJoined) onSessionJoined(response);
      } else if (response.command === 'DATA_SENT') {
        if (onDataSent) onDataSent(response);
      }
    });
